...
// tracereader.h and log.h both C headers with no ext C guard
// we wrap them here so the linker finds the right symbols
extern "C" {
...
static bool parseArgs(int argc, char* argv[], Options& opts) {
        ...
        switch (option)
        {
            case 'n':
                ...
                if (opts.maxAddresses <= 0) {
                    fprintf(stderr, "bad -n value (need > 0)\n");
                    return false;
                }
                break;
            ...
            case 'c':
                ...
                if (opts.tlbCapacity < 0) {
                    fprintf(stderr, "-c can't be negative\n");
                    return false;
                }
                break;
            case 'o':
    ...
    if (totalBits > MAX_PAGE_INDEX_BITS) {
        ...
        fprintf(stderr, "too many bits (max 28)\n");
        return false;
    }
...
static unsigned int runSim(FILE* traceFile,
                           PageTable& pageTable,
                           TLB* tlb,
                           const Options& opts,
...
int main(int argc, char* argv[]) {
    ...
    unsigned int addressesProcessed =
        runSim(traceFile, pageTable, tlb, opts,
